<div class="col-span-6 sm:{{ $field->span }}" x-data="{open: false}" @click.away="open = false" @close.stop="open = false">
  <label id="listbox-label" class="block text-sm font-medium text-gray-700">
    {{ $field->label }}
  </label>
  <div class="mt-1 relative">
    <button  @click="open=true" type="button" aria-haspopup="listbox" aria-expanded="true" aria-labelledby="listbox-label" class="relative w-full bg-white border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
      ...
      <ul tabindex="-1" role="listbox" aria-labelledby="listbox-label" aria-activedescendant="listbox-item-3" class="max-h-56 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
        @if($field->options)
            @foreach($field->options as $value => $label)
                <li @click="open = false" id="listbox-item-{{$value}}" role="option" class="text-gray-900 cursor-default select-none relative py-2 pl-3 pr-9 {{ $this->isSingleSelected($value,$field->name, ' bg-gray-200') }}"
