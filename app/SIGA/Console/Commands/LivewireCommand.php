...
{
    protected $signature = 'livewire:_module {name} {module=default}  {--force} {--inline} {--stub=default}';

    protected $description = 'Gerar componente livewire para modules';

    public function handle()
    {
        if ($this->argument('module') != 'default'){

            config([
               'livewire.class_namespace' => sprintf('App\\Modules\\%s\\App\\Http\\Livewire', $this->argument('module')),
               'livewire.view_path' => app_path(sprintf('Modules/%s/resources/views/livewire', $this->argument('module')))
            ]);
        }
