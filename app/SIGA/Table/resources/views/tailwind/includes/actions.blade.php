<div class="flex">
    @foreach($this->actions_table() as $action)
        @if($this->actionLink($model, $action['action']))
            <a href="{{ $this->actionLink($model, $action['action']) }}" class="inline-flex items-center px-4 py-2 border border-{{$action['color']}}-300 rounded-md shadow-sm text-sm font-medium text-{{$action['color']}}-700 bg-white hover:bg-{{$action['color']}}-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-{{$action['color']}}-500 mr-2"><x-c-icon class="-ml-1 mr-1 h-5 w-5 text-gray-500" fill="currentColor" icon="{{$action['icon']}}"/><span class="ml-2 hidden md:block">{{$action['text']}}</span></a>
        @endif
    @endforeach
